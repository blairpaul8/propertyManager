...
pptx_file = os.path.expanduser("~/Desktop/item_photos/item_photos.pptx")
    ...
    for shape in slide.shapes:
        if shape.has_table: 
            ...
            if len(table.rows) >= 4 and len(table.columns) >= 2:
                return table.cell(3, 1).text.strip()  # Rows/columns are 0-indexed
    return None

for slide_index, slide in enumerate(presentation.slides):
    file_name_number = get_number_from_table(slide)
    if not file_name_number:
        print(f"No number found in the 4th row, 2nd column on slide {slide_index + 1}. Skipping...")
        continue
